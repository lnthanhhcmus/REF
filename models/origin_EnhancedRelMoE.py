"""
Enhanced RelMoE with Adaptive Temperature Scaling and Cross-Modal Attention
Minimal modifications to original MoMoK for improved performance
"""
...
class EnhancedRelMoE(BaseModel):
    ...
    def __init__(self, args):
        ...
        # Store args for enhanced features
        ...
        if args.pre_trained:
            ...
            self.relation_embeddings = nn.Embedding.from_pretrained(torch.cat((
                ...
                -1 * torch.from_numpy(pickle.load(open('datasets/' + args.dataset + '/gat_relation_vec.pkl', 'rb'))).float()), 
                dim=0), freeze=False)
